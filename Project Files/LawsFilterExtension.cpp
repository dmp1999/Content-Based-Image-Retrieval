...
void getFeatureSquare(Mat &src, Mat &feature)
{

    int x = 0, y = 0;

    for (int i = src.rows / 2 - 40; i < src.rows / 2 + 50; i++)
    {
        y = 0;
        for (int j = src.cols / 2 - 40; j < src.cols / 2 + 50; j++)
        {
            for (int c = 0; c < 3; c++)
            {
...
// Laws filter L5E5
cv::Vec3s applyHorizontalLawsE5(const cv::Mat &src, int i, int j)
{
    cv::Vec3s sum = {0, 0, 0};
    int c = 0;
    ...
    for (c = 0; c < 3; c++)
    {
        ...
        sum[c] = (-1) * src.at<cv::Vec3b>(i, j - 2)[c] + (-2) * src.at<cv::Vec3b>(i, j - 1)[c] + 0 * src.at<cv::Vec3b>(i, j)[c] + 2 * src.at<cv::Vec3b>(i, j + 1)[c] + 1 * src.at<cv::Vec3b>(i, j + 2)[c];
    }

    return (sum);
}

cv::Vec3s applyVerticalLawsL5(const cv::Mat &src, int i, int j)
{
    cv::Vec3s sum = {0, 0, 0};
    int c = 0;
    int filter_rows = 3, filter_cols = 3;
    ...
    for (c = 0; c < 3; c++)
    {
        sum[c] = (1 * src.at<cv::Vec3s>(i - 2, j)[c] + 4 * src.at<cv::Vec3s>(i - 1, j)[c] + 6 * src.at<cv::Vec3s>(i, j)[c] + 4 * src.at<cv::Vec3s>(i + 1, j)[c] + 1 * src.at<cv::Vec3s>(i + 2, j)[c])/16;
    }
...
int lawsFilter(const cv::Mat &src, cv::Mat &dst)
{
    ...
    for (i = 0; i < src.rows; i++)
    {
        ...
        {
            temp.at<cv::Vec3s>(i, j) = applyHorizontalLawsE5(src, i, j);
        }
    }
    ...
    for (i = 2; i < src.rows - 2; i++)
    {
        ...
        {
            dst.at<cv::Vec3s>(i, j) = applyVerticalLawsL5(temp, i, j);
        }
    }
